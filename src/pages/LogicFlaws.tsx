...
import { logicFlawAdapter } from '@/lib/data-adapter';
import { toast } from 'sonner';
const sampleLogicFlaws: LogicFlaw[] = [
  {
    id: "1",
    workspaceId: "ws-1",
...
export default function LogicFlawsPage() {
    const [logicFlaws, setLogicFlaws] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  const loadLogicFlaws = async () => {
    try {
      setLoading(true);
      const data = await logicFlawAdapter.getAll();
      setLogicFlaws(data);
    } catch (error) {
      console.error('Failed to load logicFlaws:', error);
      toast.error('Failed to load logicFlaws');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateLogicFlaw = async () => {
    try {
      await logicFlawAdapter.create(newLogicFlaw);
  ...
  const handleUpdateLogicFlaw = async (id: string, updates: any) => {
    try {
      await logicFlawAdapter.update(id, updates);
  ...
  const handleDeleteLogicFlaw = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this item?')) {
      try {
        await logicFlawAdapter.delete(id);
  ...
  const filteredFlaws = logicFlaws.filter(flaw => {
  ...
  const handleUpdateFlaw = () => {
    if (editingFlaw) {
      handleUpdateLogicFlaw(editingFlaw.id, editingFlaw);
    }
  };

  const handleDeleteFlaw = (flawId: string) => {
    handleDeleteLogicFlaw(flawId);
  };
  ...
  const handleSaveInNoteSpace = (updatedFlaw: LogicFlaw) => {
    handleUpdateLogicFlaw(updatedFlaw.id, updatedFlaw);
  };
