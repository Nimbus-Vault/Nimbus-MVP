...
import { payloadAdapter } from '@/lib/data-adapter';
...
export default function PayloadsPage() {
  ...
  const loadPayloads = async () => {
    try {
      setLoading(true);
      const data = await payloadAdapter.getAll();
      setPayloads(data);
    } catch (error) {
  ...
  const handleCreatePayload = async () => {
    try {
      await payloadAdapter.create(newPayload);
  ...
  const handleUpdatePayload = () => {
    ...
    const index = payloads.findIndex(p => p.id === editingPayload.id);
    if (index !== -1) {
      payloads[index] = {
        ...editingPayload,
  ...
  const handleDeletePayload = (payloadId: string) => {
    if (window.confirm('Are you sure you want to delete this payload?')) {
      ...
      // In real implementation, this would make an API call
      const index = payloads.findIndex(p => p.id === payloadId);
      if (index !== -1) {
  ...
  const handleSaveInNoteSpace = (updatedPayload: any) => {
    ...
    // In real implementation, this would make an API call
    const index = payloads.findIndex(p => p.id === updatedPayload.id);
    if (index !== -1) {
  ...
  const platforms = [...new Set(payloads.map(p => p.platform))];
