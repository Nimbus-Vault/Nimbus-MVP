...
import { techniqueAdapter } from '@/lib/data-adapter';
...
export default function TechniquesPage() {
  ...
  const loadTechniques = async () => {
    try {
      setLoading(true);
      const data = await techniqueAdapter.getAll();
      setTechniques(data);
    } catch (error) {
  ...
  const handleCreateTechnique = async () => {
    try {
      await techniqueAdapter.create(newTechnique);
  ...
  const handleUpdateTechnique = () => {
    ...
    const index = techniques.findIndex(t => t.id === editingTechnique.id);
    if (index !== -1) {
      techniques[index] = {
        ...editingTechnique,
  ...
  const handleSaveInNoteSpace = (updatedTechnique: any) => {
    ...
    // In real implementation, this would make an API call
    const index = techniques.findIndex(t => t.id === updatedTechnique.id);
    if (index !== -1) {
