...
import { platformAdapter } from '@/lib/data-adapter';
...
export default function PlatformsPage() {
  ...
  const loadPlatforms = async () => {
    try {
      ...
      const data = await platformAdapter.getAll(currentWorkspaceId);
      setPlatforms(data);
    } catch (error) {
  ...
  const handleCreatePlatform = async () => {
    try {
      ...
      await platformAdapter.create(platformData);
  ...
  const handleUpdatePlatform = async () => {
    ...
    try {
      await platformAdapter.update(editingPlatform.id, editingPlatform);
  ...
  const handleDeletePlatform = async (platformId: string) => {
    if (window.confirm('Are you sure you want to delete this platform? This action cannot be undone.')) {
      try {
        await platformAdapter.delete(platformId);
